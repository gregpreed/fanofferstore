...
class WC_Gateway_PayPal_Express extends SV_WC_Payment_Gateway_Direct {
	...
	public function get_method_form_fields() {
		...
		// TODO: remove this once using framework 3.2+ as it's included already there
		return apply_filters( 'wc_payment_gateway_paypal_express_form_fields', $fields, $this );
	}
	...
	public function render_express_checkout_button() {
		...
		if ( WC()->cart->total > 0 || WC()->cart->subtotal > 0 ) {
			...
			if ( 'image' === $this->get_checkout_button_style() ) {
				...
				$button_markup .= '<img src="https://www.paypal.com/' . $this->get_safe_locale() . '/i/btn/btn_xpressCheckout.gif" width="145" height="42" style="width: 145px; height: 42px; float: right; clear: both; margin: 3px 0 0 0; border: none; padding: 0;" align="top" alt="' . __( 'Check out with PayPal', WC_PayPal_Express::TEXT_DOMAIN ) . '" />';
				$button_markup .= "</a>";
			...
			if ( $this->bill_me_later_enabled() ) {
				...
				$button_markup .= '<img src="https://www.paypalobjects.com/webstatic/en_US/btn/btn_bml_SM.png" width="145" height="32" style="width: 145px; height: 32px; float: right; clear: both; border: none; padding: 0; margin: 0;" align="top" alt="' . __( 'Check out with PayPal', WC_PayPal_Express::TEXT_DOMAIN ) . '" />';
				$button_markup .= '</a>';
		...
		if ( ! $this->is_express_checkout() ) {
			...
			if ( is_ajax() ) {
				...
				if ( SV_WC_Plugin_Compatibility::is_wc_version_gte_2_4() ) {
					...
					// WC 2.4 no longer prefixes it's JSON
					wp_send_json( $args );
