...
"use strict";
		...
		if ( $('.wcpf_mode_presets').length > 0 ) {
			...
			curr.find('p span[class^=wcpfs_]').each( function() {
				...
				if ( $('.prdctfltr_customizer_fields').find('a.pf_active[data-filter="'+curr_label_stripped+'"]').length > 0 ) {
					...
					curr_el.prev().addClass(curr_label+'_init').css({'visibility':'hidden','position':'absolute','width':'0','height':'0'});
					curr_el.addClass(curr_label+'_init').css({'visibility':'hidden','position':'absolute','width':'0','height':'0'});
					curr_el.next().addClass(curr_label+'_init').css({'visibility':'hidden','position':'absolute','width':'0','height':'0'});
				}
			});
		}
	}
	...
	$(document).on('click', '.prdctfltr_c_add_filter', function() {
		...
		if ( curr_el.hasClass('pf_active') ) {
			...
			curr_set.prev().removeAttr('style').css({'visibility':'hidden','position':'absolute','width':'0','height':'0'});
			curr_set.removeAttr('style').css({'visibility':'hidden','position':'absolute','width':'0','height':'0'});
			curr_set.next().removeAttr('style').css({'visibility':'hidden','position':'absolute','width':'0','height':'0'});
	...
	$(document).on('click', '.prdctfltr_c_add.pf_advanced', function() {
		...
		$.post(prdctfltr.ajax, curr_data, function(response) {
			if (response) {
				...
				var adv_ui = '<div class="pf_element adv" data-filter="advanced" data-id="'+response[0]+'"><span>Advanced Filter</span><a href="#" class="prdctfltr_c_delete"><i class="prdctfltr-delete"></i></a><a href="#" class="prdctfltr_c_move"><i class="prdctfltr-move"></i></a><a href="#" class="prdctfltr_c_toggle"><i class="prdctfltr-down"></i></a><div class="pf_options_holder">'+response[1]+'</div></div>';
				...
				if ( curr_preset['wc_settings_prdctfltr_active_filters'] !== undefined ) {
					...
					$.each(curr_preset['wc_settings_prdctfltr_active_filters'], function(index, pf_filter) {
						...
						if ( pf_filter == 'advanced' ) {
							...
							curr_el.append('<div class="pf_element adv" data-filter="'+pf_filter+'" data-id="'+curr+'"><span>Advanced Filter</span><a href="#" class="prdctfltr_c_delete"><i class="prdctfltr-delete"></i></a><a href="#" class="prdctfltr_c_move"><i class="prdctfltr-move"></i></a><a href="#" class="prdctfltr_c_toggle"><i class="prdctfltr-down"></i></a><div class="pf_options_holder"></div></div>');
						...
						if ( !--curr_count ) {
							$('.prdctfltr_customizer_fields .prdctfltr_c_add_filter:not(.pf_active)').each( function() {
								...
								curr_set.prev().addClass('wcpfs_'+curr_filter+'_init').css({'visibility':'hidden','position':'absolute','width':'0','height':'0'});
								curr_set.addClass('wcpfs_'+curr_filter+'_init').css({'visibility':'hidden','position':'absolute','width':'0','height':'0'});
								curr_set.next().addClass('wcpfs_'+curr_filter+'_init').css({'visibility':'hidden','position':'absolute','width':'0','height':'0'});
								curr_set.next().find('select, input').prop('disabled', false);
							});
						}
