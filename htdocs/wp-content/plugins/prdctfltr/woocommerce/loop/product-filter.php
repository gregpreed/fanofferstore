	...
	<?php
					...
					default :
						...
						if ( in_array( $mod, array( 'regular', 'attribute' ) ) ) {
							...
							$curr_fo['settings']['adoptive'] = $curr_options['wc_settings_prdctfltr_' . $v . '_adoptive'];
							...
							$curr_fo['settings']['multi'] = $curr_options['wc_settings_prdctfltr_advanced_filters']['pfa_multi'][$n];
					...
					if ( !empty( $catalog_categories ) && !is_wp_error( $catalog_categories ) ) {
						...
						$curr_include = WC_Prdctfltr::prdctfltr_wpml_include_terms( $curr_include, $curr_fo['filter'] );

					?>
						<div class="prdctfltr_filter prdctfltr_attributes prdctfltr_<?php echo $v; ?> <?php echo $curr_term_style; ?><?php echo $curr_term_multi; ?><?php echo $curr_term_adoptive; ?><?php echo $curr_term_relation; ?><?php echo $curr_term_expand; ?>" data-filter="<?php echo $curr_fo['filter']; ?>">
							...
							<?php
								...
								foreach ( $catalog_categories as $term ) {
									...
									switch ( $curr_term_style ) {
										case 'pf_attr_text':
											$curr_insert = $term->name . ( $curr_options['wc_settings_prdctfltr_show_counts'] == 'no' || $term->count == '0' ? '' : ' <span class="prdctfltr_count">' . ( isset($output_terms[$curr_fo['filter']]) && isset($output_terms[$curr_fo['filter']][$term->slug]) && $output_terms[$curr_fo['filter']][$term->slug] != $term->count ? $output_terms[$curr_fo['filter']][$term->slug] . '/' . $term->count : $term->count ) . '</span>' );
										break;
										case 'pf_attr_imgtext':
										...
										case 'pf_attr_img':
											...
											$curr_insert = ( $curr_img !== '' ? $curr_img : '<img src="' . WC_Prdctfltr::$url_path . '/lib/images/pf-placeholder.gif" />');
											$curr_insert .= '<span class="prdctfltr_tooltip"><span>' . $term->name . ( $curr_options['wc_settings_prdctfltr_show_counts'] == 'no' || $term->count == '0' ? '' : ' <span class="prdctfltr_count">' . ( isset($output_terms[$curr_fo['filter']]) && isset($output_terms[$curr_fo['filter']][$term->slug]) && $output_terms[$curr_fo['filter']][$term->slug] != $term->count ? $output_terms[$curr_fo['filter']][$term->slug] . '/' . $term->count : $term->count ) . '</span>' ) . '</span></span>';
										break;
										default :
												...
												foreach( $sub->children as $subsub ) {
													...
													if ( !empty($subsub->children) ) {
														...
														foreach( $subsub->children as $subsubsub ) {
															...
															printf('<label class="%5$s%4$s%6$s"><input type="checkbox" value="%1$s"%3$s /><span>%2$s</span></label>', $decode_slug, $subsubsub->name, ( in_array( $decode_slug, $curr_cat_selected ) ? ' checked' : '' ), ( in_array( $decode_slug, $curr_cat_selected ) ? ' prdctfltr_active' : '' ), $pf_adoptive_class, ( !empty($subsubsub->children) && in_array( $decode_slug, $curr_cat_selected ) ? ' prdctfltr_clicked' : '' ) );
