...
class WC_Brands {
	...
	public function __construct() {
		...
		add_filter( 'woocommerce_coupon_is_valid_for_product', array( $this, 'validate_product_brands_is_valid_for_product' ), null, 4 );
    ...
    /**
     * Validate a coupon code based on the brands in the cart.
     * @access public
     * @since  1.3.0
     * @param  boolean $bool     Valid or not.
     * @param  object  $this_obj Cart object.
     * @return boolean
     */
    public function validate_coupon ( $bool, $this_obj ) {
    	$this->_validate_cart_product_brands( $this_obj );
    	$this->_validate_cart_excluded_product_brands( $this_obj );
    	return $bool;
    } // End validate_coupon()
     ...
     * @return  void
     */
    public function validate_product_brands_is_valid_for_product ( $valid, $product, $this_obj, $values ) {
    	...
    	return $valid;
    } // End validate_product_brands_is_valid_for_product()

    /**
	 * Ensure coupon is valid for product brands in the cart is valid or throw exception
	 */
	private function _validate_cart_product_brands( $this_obj ) {

		$stored_product_brands = array();

		if ( get_post_meta( $this_obj->id, 'product_brands', true ) ) {
			$stored_product_brands = (array) get_post_meta( $this_obj->id, 'product_brands', true );
		}

		if ( sizeof( $stored_product_brands ) > 0 && ! $this_obj->is_type( array( 'fixed_product', 'percent_product' ) ) ) {
			$valid_for_cart = false;
			if ( sizeof( WC()->cart->get_cart() ) > 0 ) {
				foreach( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {

					$product_cats = wp_get_post_terms( $cart_item['product_id'], 'product_brand', array( "fields" => "ids" ) );

					if ( sizeof( array_intersect( $product_cats, $stored_product_brands ) ) > 0 ) {
						$valid_for_cart = true;
					}
				}
			}
			if ( ! $valid_for_cart ) {
				throw new Exception( $this_obj::E_WC_COUPON_NOT_APPLICABLE );
			}
		}
	}
	 ...
	 */
	private function _validate_cart_excluded_product_brands( $this_obj ) {
		$excluded_product_brands = (array)get_post_meta( $this_obj->id, 'exclude_product_brands', true );
		if ( sizeof( $excluded_product_brands ) > 0 ) {
			$valid_for_cart = true;
			if ( sizeof( WC()->cart->get_cart() ) > 0 ) {
				foreach( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
					...
					if ( sizeof( array_intersect( $product_cats, $excluded_product_brands ) ) > 0 ) {
						$valid_for_cart = false;
					}
				}
			}
