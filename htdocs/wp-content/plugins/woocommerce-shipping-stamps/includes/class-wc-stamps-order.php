...
class WC_Stamps_Order {
	...
	public function __construct() {
		...
		$this->package_types = array(
			...
			'Regional Rate Box C'       => __( 'USPS regional rate box C. A special 15" x 12" x 12" USPS box that clearly indicates "Regional Rate Box C". 25 lbs maximum weight.', 'woocommerce-shipping-stamps' ),
			'Legal Flat Rate Envelope'  => __( 'USPS flat rate padded envelope.', 'woocommerce-shipping-stamps' ),
			'Flat Rate Padded Envelope' => __( 'USPS-supplied Priority Mail flat-rate envelope 9 1/2" x 15". Maximum weight 4 pounds.', 'woocommerce-shipping-stamps' )
		);
	}
	...
	public function get_packages_html( $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$total_weight += wc_get_weight( $weight * $item['qty'], 'lbs' );
