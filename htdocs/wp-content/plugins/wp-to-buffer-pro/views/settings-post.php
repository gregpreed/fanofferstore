...
<div id="<?php echo $post_type; ?>-panel" class="panel">
	...
	<!-- Second level tabs -->
	<h3 class="nav-tab-wrapper needs-js">  
		<!-- Default Settings -->
		<a href="#<?php echo $post_type; ?>-default" class="nav-tab nav-tab-active">
			<?php _e( 'Defaults', $this->plugin->name ); ?><br /><br />
		</a>
        ...
        foreach ( $profiles as $key => $profile ) {
           ?>
            <a href="#<?php echo $post_type; ?>-<?php echo $profile['id']; ?>" class="nav-tab image" title="<?php echo $profile['formatted_service'] . ': ' . $profile['formatted_username']; ?>">
	...
	<div id="<?php echo $post_type; ?>-<?php echo $profile_id; ?>-panel" class="sub-panel">
        ...
        foreach ( $post_actions as $action => $action_label ) {
            require( $this->plugin->folder . 'views/settings-post-action.php' );
        }
        ?>
    </div>
    <!-- /Defaults -->
    ...
    foreach ( $profiles as $key => $profile ) {
        ...
        <div id="<?php echo $post_type; ?>-<?php echo $profile_id; ?>-panel" class="sub-panel">
            <?php
            require( $this->plugin->folder . 'views/settings-post-actionheader.php' );
            ?>

            <div id="<?php echo $post_type; ?>-<?php echo $profile_id; ?>-actions-panel">
                <?php
                // Iterate through Post Actions (Publish, Update etc)
                foreach ( $post_actions as $action => $action_label ) {
                    // Load sub view
