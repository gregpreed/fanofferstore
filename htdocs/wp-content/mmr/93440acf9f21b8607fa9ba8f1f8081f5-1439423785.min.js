...
function switch_search_buttons(){if($(".site-search .search-field").val()!==""){$(".search-but-added").css("z-index","1");$(".site-search input[type=\"submit\"]").css("z-index","2")}else{$(".search-but-added").css("z-index","2");$(".site-search input[type=\"submit\"]").css("z-index","1")}}
...
$("#searchform div, .site-search div").append('<div class="search-but-added"><i class="getbowtied-icon-search"></i></div>');$(".search-button, .search-but-added").click(function(){$(".site-search").toggleClass("open");toggle_logo_nav();switch_search_buttons();$("body").on('click',function(e){if($(e.target).attr('class')=='getbowtied-icon-search'||$(e.target).attr('class')=='search-field'){return}else{reset_search_toggles()
...
disable_fresco();function handleNavigation(){setTimeout(function(){if($(window).innerWidth()>1400){if($(".single_product_summary_related.on_screen, #site-footer.on_screen")[0]){$(".product-nav-previous, .product-nav-next").hide()}else{$(".product-nav-previous, .product-nav-next").fadeIn(300)}}else{$(".product-nav-previous, .product-nav-next").hide()}},100)}
handleNavigation();function handleSelect(){if($(window).innerWidth()>1024){$(".orderby, .big-select, .topbar-language-switcher, .wcml_currency_switcher").select2({allowClear:!0,minimumResultsForSearch:Infinity})}}
...
var product_thumbnails_swiper=$('.product_thumbnails .swiper-container').swiper({slidesPerView:slides_per_view,watchActiveIndex:!0,mousewheelControl:!0,mode:product_thumbnails_swiper_mode,onSlideClick:function(swiper){owl.goTo(product_thumbnails_swiper.clickedSlideIndex);for(var i=0;i<product_thumbnails_swiper.slides.length;i++){product_thumbnails_swiper.slides[i].style.opacity=0.2}
product_thumbnails_swiper.slides[product_thumbnails_swiper.clickedSlideIndex].style.opacity=1;product_thumbnails_swiper.swipeTo(product_thumbnails_swiper.clickedSlideIndex,300,'')}});$(".featured_img_temp").hide();$("#product-images-carousel").owlCarousel({singleItem:!0,autoHeight:!0,transitionStyle:"fade",lazyLoad:!0,afterAction:afterAction,});var owl=$("#product-images-carousel").data('owlCarousel');function afterAction(){if($(".product_thumbnails").length){for(var i=0;i<product_thumbnails_swiper.slides.length;i++){product_thumbnails_swiper.slides[i].style.opacity=0.2}
...
$(".single_product_summary_related, #site-footer").each(function(i,el){if($(el).visible(!0)){$(el).addClass("on_screen");handleNavigation()}else{$(el).removeClass("on_screen");handleNavigation()}});parallax_engine($(this).scrollTop());refreshBackgrounds(".st-content")});$(document).ajaxComplete(function(event,request,settings){$(".products li").addClass("animate");$(".product_thumbnail.with_second_image .product_thumbnail_background").css("background-size","cover");$(".product_thumbnail.with_second_image").addClass("second_image_loaded")})});(function(e){e.fn.hoverIntent=function(t,n,r){var i={interval:100,sensitivity:7,timeout:0};if(typeof t==="object"){i=e.extend(i,t)}else if(e.isFunction(n)){i=e.extend(i,{over:t,out:n,selector:r})}else{i=e.extend(i,{over:t,out:t,selector:n})}var s,o,u,a;var f=function(e){s=e.pageX;o=e.pageY};var l=function(t,n){n.hoverIntent_t=clearTimeout(n.hoverIntent_t);if(Math.abs(u-s)+Math.abs(a-o)<i.sensitivity){e(n).off("mousemove.hoverIntent",f);n.hoverIntent_s=1;return i.over.apply(n,[t])}else{u=s;a=o;n.hoverIntent_t=setTimeout(function(){l(t,n)},i.interval)}};var c=function(e,t){t.hoverIntent_t=clearTimeout(t.hoverIntent_t);t.hoverIntent_s=0;return i.out.apply(t,[e])};var h=function(t){var n=jQuery.extend({},t);var r=this;if(r.hoverIntent_t){r.hoverIntent_t=clearTimeout(r.hoverIntent_t)}if(t.type=="mouseenter"){u=n.pageX;a=n.pageY;e(r).on("mousemove.hoverIntent",f);if(r.hoverIntent_s!=1){r.hoverIntent_t=setTimeout(function(){l(n,r)},i.interval)}}else{e(r).off("mousemove.hoverIntent",f);if(r.hoverIntent_s==1){r.hoverIntent_t=setTimeout(function(){c(n,r)},i.timeout)}}};return this.on({"mouseenter.hoverIntent":h,"mouseleave.hoverIntent":h},i.selector)}})(jQuery);(function($){'use strict';function tfingi_megamenu_init(){$('.megamenu-parent').each(function(index,el){var megamenu_parent=$(this);var megamenu=$(megamenu_parent).children('.sub-menu');megamenu.css({width:''});var megamenu_columns_count=megamenu.children('.megamenu-column').length;var megamenu_width=megamenu_parent.attr('data-width');var megamenu_left='';var megamenu_parent_itemwidth=0;$(megamenu).addClass('megamenu-sub-menu');$(megamenu).find('.sub-menu').each(function(){$(this).attr('class','').addClass('megamenu-inner-sub-menu')});$(megamenu).find('.megamenu-column').each(function(){$(this).find('.megamenu-inner-sub-menu .megamenu-heading').eq(0).addClass('first-submenu-header')});if(megamenu_width===''){megamenu_width=260*megamenu_columns_count}
if(megamenu_width==='full'){megamenu_left='0';megamenu.css({width:'100%','margin-left':megamenu_left})}else{$(this).css('position','relative');megamenu_parent_itemwidth=$(megamenu_parent).outerWidth();megamenu_left=parseInt(megamenu_width,10)/2*(-1)+megamenu_parent_itemwidth/2;$(megamenu).css({width:megamenu_width+'px','margin-left':megamenu_left+'px'})}
megamenu_parent.addClass('megamenu-columns-'+megamenu_columns_count);var window_width=jQuery(window).width();var megamenu_parent_offset=$(megamenu_parent).offset();if(megamenu_width!='full'){if((window_width<megamenu_width)){$(this).addClass('mm-width-full');megamenu_left='0';megamenu.css({width:'100%',position:'static','margin-left':megamenu_left})}else if(0>(megamenu_parent_offset.left-megamenu_width)){megamenu.css({'margin-left':0})}else if(window_width<(megamenu_parent_offset.left+(megamenu_parent_itemwidth/2)+megamenu_width/2)){$(megamenu_parent).addClass('dropdown-align-right');megamenu.css({'margin-left':'0','right':'0','left':'auto'})}}})}
tfingi_megamenu_init();$(window).resize(tfingi_megamenu_init)})(jQuery);document.documentElement.className+=' js_active ';document.documentElement.className+='ontouchstart' in document.documentElement?' vc_mobile ':' vc_desktop ';(function(){var prefix=['-webkit-','-o-','-moz-','-ms-',""];for(var i=0;i<prefix.length;i++){if(prefix[i]+'transform' in document.documentElement.style){document.documentElement.className+=" vc_transform "}}})();jQuery(window).load(function(){});var vc_js=function(){vc_twitterBehaviour();vc_toggleBehaviour();vc_toggleBehaviourOld();vc_tabsBehaviour();vc_accordionBehaviour();vc_teaserGrid();vc_carouselBehaviour();vc_slidersBehaviour();vc_prettyPhoto();vc_googleplus();vc_pinterest();vc_progress_bar();vc_plugin_flexslider();vc_google_fonts();vc_gridBehaviour();vc_rowBehaviour();vc_ttaActivation();jQuery(document).trigger('vc_js');window.setTimeout(vc_waypoints,1500)};jQuery(document).ready(function($){window.vc_js()});if(typeof window.vc_plugin_flexslider!=='function'){window.vc_plugin_flexslider=function($parent){var $slider=$parent?$parent.find('.wpb_flexslider'):jQuery('.wpb_flexslider');$slider.each(function(){var this_element=jQuery(this);var sliderSpeed=800,sliderTimeout=parseInt(this_element.attr('data-interval'))*1000,sliderFx=this_element.attr('data-flex_fx'),slideshow=!0;if(sliderTimeout==0){slideshow=!1}
...
$('.vc_parallax-inner').remove();$('[data-5p-top-bottom]').removeAttr('data-5p-top-bottom data-30p-top-bottom');$('[data-vc-parallax]').each(function(){var skrollrSpeed,skrollrSize,skrollrStart,skrollrEnd,$parallaxElement,parallaxImage,youtubeId;callSkrollInit=!0;if($(this).data('vcParallaxOFade')=='on'){$(this).children().attr('data-5p-top-bottom','opacity:0;').attr('data-30p-top-bottom','opacity:1;')}
